...
GitHub Releases API を利用した自動アップデート機能。ポータブルアプリ（単一 exe）に適したバッチファイル方式を採用。

| 項目 | 内容 |
|---|---|
| バージョン管理 | `version.py` にセマンティックバージョンとリポジトリ情報を定義 |
| チェック先 | `https://api.github.com/repos/{repo}/releases/latest` |
| 定期チェック | 6 時間間隔（QTimer）+ 起動 30 秒後の初回チェック |
| 手動チェック | 設定画面の About タブ、またはトレイメニューから実行可能 |
| 自動チェック | 設定で有効/無効を切り替え可能（デフォルト: 有効） |

**更新フロー:**

1. GitHub API で最新リリースの `tag_name` を取得
2. セマンティックバージョンで現在のバージョンと比較
3. リリースアセットから `.exe` ファイルを特定
4. 確認ダイアログを表示（バージョン番号を提示）
5. ユーザーが承認 → バックグラウンドでダウンロード（進捗表示あり）
6. ダウンロード完了 → バッチファイルを生成
7. 現在のプロセスを終了
8. バッチファイルが旧 exe → `.bak`、新 exe → 元の名前に置き換え、再起動
9. バッチファイルが `.bak` と自分自身を削除

**シグナル（AutoUpdater → UI）:**

| シグナル | 用途 |
|---|---|
| `update_found(UpdateInfo)` | 新バージョン検出 |
| `update_not_found()` | 最新版使用中 |
| `update_error(str)` | エラー発生 |
...
main() 開始
  ...
  │    ├─ 翻訳エンジン作成
  │    ├─ オーバーレイ初期化
  │    ├─ 設定画面初期化（AutoUpdater を接続）
  │    ├─ ホットキーマネージャー開始
  │    └─ システムトレイ表示（アップデート確認メニュー含む）
  │
  ├─ 初回起動: 自動起動設定ダイアログ
  │
  ├─ 自動アップデートチェック開始（設定で有効な場合）
  │
  └─ Qt イベントループ開始
```
...
プラットフォーム抽象化レイヤー（`native/`）の `create_single_instance_lock()` を使用。

**Windows 実装:**

| 技術的詳細 | 内容 |
|---|---|
| 方式 | 名前付き Mutex（`Global\ChatBridge_SingleInstance_Mutex`） |
| 名前空間 | `Global\`（セッション横断） |
...
プラットフォーム抽象化レイヤーの `is_admin()` / `relaunch_as_admin()` を使用。
...
├── updater.py           # 自動アップデートモジュール（GitHub Releases）
...
│   ├── __init__.py      # プラットフォーム検出・自動選択（シングルトン）
...
├── version_info.txt     # PyInstaller 用 Windows バージョン情報
...
`native/` モジュールにより、OS 固有の処理を抽象化している。
将来の macOS 対応（Phase 2.5）に備えた設計。

### 8.1 アーキテクチャ

```
native/
├── __init__.py      # get_platform() — シングルトンで適切な実装を返す
├── base.py          # BasePlatform（抽象基底クラス）
└── windows.py       # WindowsPlatform（Windows 固有実装）
```

### 8.2 BasePlatform インターフェース

| メソッド | 用途 |
|---|---|
| `is_admin()` | 管理者/特権権限の確認 |
| `relaunch_as_admin()` | 管理者権限での再起動 |
| `create_single_instance_lock()` | 二重起動防止ロックの作成 |
| `get_auto_start()` | 自動起動の登録状態を確認 |
| `set_auto_start(enabled)` | 自動起動の登録/解除 |
| `show_window_no_activate(handle)` | フォーカスを奪わずにウィンドウ表示 |
| `is_modifier_pressed()` | 修飾キーの物理押下状態を確認 |
| `get_exe_path()` | 実行ファイルのパスを取得 |

### 8.3 プラットフォーム自動選択

`native.get_platform()` が `sys.platform` に基づいて適切な実装を返す。
新しいプラットフォームを追加する場合は、`BasePlatform` を継承した具象クラスを作成し、
`__init__.py` に分岐を追加するだけでよい。
...
5. **自動アップデート**: 開発環境（`python main.py`）では exe の置き換えが行われない（ダウンロードまでは動作する）
